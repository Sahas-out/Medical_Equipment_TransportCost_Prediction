...
val_pred = best_elastic.predict(X_val)
...
# Show feature selection (non-zero coefficients)
selected_features = np.sum(best_elastic.coef_ != 0)
print(f"Features selected: {selected_features}")

# Final predictions on test data
test_pred = best_elastic.predict(X_test)
...
predictions_df.to_csv('output/elasticnet_predictions.csv', index=False)
print("Predictions saved to output/elasticnet_predictions.csv")
